...
// ── 微信跳转链还原 ──
//
// 短链 /s/<hash> 在手机端发布时不被微信识别，<a> 整个被换成 <span leaf="">。
// 微信自己插入文章链接时用的是 /s?__biz=…&mid=…&idx=…&sn=…&scene=21#wechat_redirect
// （不带 chksm），而这四个参数在文章页里都埋着。
const { jumpUrlFromPage, toJumpUrl } = require('../mp-article');

const PAGE = `
  var biz = "MzAxMDMxOTI2NA==" || "";
  var mid = "2649111825" || "";
  var idx = "1" || "";
  var sn = "dd3bb20eaba3beec444a5260a9ec0b61" || "";
`;

test('jumpUrlFromPage: 还原出的链接与微信自己生成的一致', () => {
  // 这个期望值是从微信实际发出来的文章里抄的，不是推出来的
  assert.strictEqual(
    jumpUrlFromPage(PAGE),
    'https://mp.weixin.qq.com/s?__biz=MzAxMDMxOTI2NA==&mid=2649111825&idx=1'
      + '&sn=dd3bb20eaba3beec444a5260a9ec0b61&scene=21#wechat_redirect',
  );
});

test('jumpUrlFromPage: __biz 末尾的 == 不能被编码', () => {
  // 编码成 %3D%3D 微信会 302 到「未知错误」页
  assert.ok(jumpUrlFromPage(PAGE).includes('__biz=MzAxMDMxOTI2NA=='));
});

test('jumpUrlFromPage: 缺任何一个参数都返回 null，不拼半截链接', () => {
  for (const drop of ['biz', 'mid', 'idx', 'sn']) {
    const broken = PAGE.replace(new RegExp(`var ${drop} = "[^"]*"`), `var ${drop} = ""`);
    assert.strictEqual(jumpUrlFromPage(broken), null, `缺 ${drop} 时应返回 null`);
  }
  assert.strictEqual(jumpUrlFromPage(''), null);
});

test('toJumpUrl: 长链只补后缀，不需要抓页面', async () => {
  const long = 'https://mp.weixin.qq.com/s?__biz=A==&mid=1&idx=1&sn=s&chksm=c';
  assert.strictEqual(await toJumpUrl(long), long + '&scene=21#wechat_redirect');
});

test('toJumpUrl: 已经带 #wechat_redirect 的不重复追加', async () => {
  const done = 'https://mp.weixin.qq.com/s?__biz=A==&mid=1&idx=1&sn=s&scene=21#wechat_redirect';
  assert.strictEqual(await toJumpUrl(done), done);
});

test('toJumpUrl: 非微信链接原样返回，不去抓', async () => {
  assert.strictEqual(await toJumpUrl('https://example.com/x'), 'https://example.com/x');
});
