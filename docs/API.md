...
> **`chksm` 参数不能省。** 实测去掉它，微信只返回一段约 3.5KB 的 JS 壳，
> 拿不到正文；带上才是完整页面（约 3.5MB）。从微信后台复制链接时整条贴进来即可，
> `scene`、`sessionid` 这类跟踪参数服务端会自己清掉。
>
...
| `请提供文章链接` | 缺 `url` |
| `不是有效的公众号文章链接（需要 mp.weixin.qq.com/s?... 形式）` | URL 格式不对 |
